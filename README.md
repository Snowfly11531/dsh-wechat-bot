...
不需要打开电脑,直接在微信里给机器人发切换指令,消息会被**先拦截**由默认 AI 模型判断是否切换请求:

- **指定目标**:发「切换到 会话「微信助手」」或「切到 D:\proj 的 微信助手」→ 直接切换并收到确认
- **只指定工作区**(或只说"切换会话"):机器人列出该工作区(或全部)的会话列表,回复**序号或标题**即可完成切换
...
Bot: 📂 请选择要切换到的会话 (回复序号或标题):
     1. 📁 D:\proj → 会话「微信助手」
     2. 📁 D:\proj → 会话「dsh更新状态查询」
     3. 📁 C:\work → 会话「周报」
你: 3
Bot: ✅ 已切换: C:\work → 会话「周报」
     之后的微信消息都会进入这个会话。
```

> 关键词粗筛(切换/切到/换到/换工作区/换会话/workspace/session 等)命中才调用模型判断,
> 普通消息不受影响;模型判断失败时自动降级为正常对话,不会卡住聊天。
