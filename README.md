...
- **切换前自动总结**:选定目标会话后,机器人先用 AI 总结该会话的历史(主题/进展/待办),随确认消息一起发回微信——切过去之前先看清那边在聊什么
...
> **只有「切换会话」「切换工作区」这两个精确词会触发切换流程**(容忍句末标点如"切换会话。")。
> 其他任何消息(包括"切到 xx""换到 xx"等说法)一律进入当前绑定的会话正常聊天,
> 不会被拦截,也不消耗额外的模型调用。
> 模型/推理等级/权限模式由 DSH 全局配置控制(设置 → 模型、sandbox-policy),插件不干预。
