...
- 发 **「切换模型」**:列出可用模型(provider/模型名),选定后接着选择**推理等级**(off/high/max),之后该微信会话的消息使用新模型
- 发 **「切换权限」**:选择权限模式——**只读 / 工作区写入 / 所有权限**(对应 DSH SandboxMode),对该微信会话生效并持久化
...
Bot: 📂 请选择权限模式 (回复序号或名称):
     1. 只读 (read-only)
     2. 工作区写入 (workspace-write)
     3. 所有权限 (danger-full-access)
你: 2
Bot: ✅ 已切换权限模式: 工作区写入 (workspace-write)
     之后的微信消息将在此权限下运行。
```

> **只有「切换会话」「切换工作区」「切换模型」「切换权限」这四个精确词会触发切换流程**(容忍句末标点如"切换会话。")。
