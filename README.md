...
- 发 **「当前工作区」** / **「当前会话」** / **「当前模型」**:查询当前绑定状态(工作区名/路径、会话标题与 id)与当前模型/推理等级(标注是会话覆盖还是全局默认)
- 发 **「统计用量」**:统计**当前模型对应平台**的用量与余额/额度(复用 dsh-usage-stats:账户快照 + 额度窗口 + 今日 token 用量)
...
> **只有「切换会话」「切换工作区」「新增工作区」「新增会话」「切换模型」「当前工作区」「当前会话」「当前模型」「统计用量」这九个精确词会触发拦截**(容忍句末标点如"切换会话。")。
