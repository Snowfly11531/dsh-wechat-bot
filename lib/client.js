...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"settings.rootDirHint": "「新增工作区」将在此目录下创建同名文件夹 (例如 D:\\proj)",
		...
		const en = {
			...
			"settings.rootDirHint": "New workspaces are created as same-name folders under this directory (e.g. D:\\proj)",
		...
		/**
		 * 扫码登录 + 运行状态内嵌组件: 轮询 <statusPath>/json。
		 */
		function WechatStatusPanel({ t, statusPath }) {
			const [state, setState] = react.useState({ status: "idle", qrcode: null, error: null, user: null });
			const [busy, setBusy] = react.useState(false);
			const refresh = react.useCallback(() => {
				fetch(statusPath + "/json", { headers: { accept: "application/json" } })
			...
			const requestQr = () => {
				setBusy(true);
				fetch(statusPath + "/login", { method: "POST" })
					.then((r) => r.json())
					.then(() => refresh())
		 ...
		 * props: { t, statusPath, chatId, onBound, liveBindings }
		 */
		function WechatBindingPicker({ t, statusPath, chatId, onBound, liveBindings }) {
			...
			const refresh = () => {
				fetch(statusPath + "/workspaces", { headers: { accept: "application/json" } })
					.then((r) => (r.ok ? r.json() : null))
					.then((data) => {
		 ...
		 * props: { t, value, save, saving, statusPath }
		 */
		function WechatSettingsPage({ t, value, save, saving, statusPath }) {
			...
			const onCreateWorkspace = () => {
				...
				setWsBusy(true);
				fetch(statusPath + "/new-workspace", {
					method: "POST",
					headers: { "content-type": "application/json" },
			...
			return react_jsx_runtime.jsx("section", {
				...
				children: [
					...
					react_jsx_runtime.jsx(WechatStatusPanel, { t, statusPath }),
					...
					react_jsx_runtime.jsx("div", {
						...
						children: [
							...
							react_jsx_runtime.jsx("label", {
								style: pageStyle.row,
								children: [react_jsx_runtime.jsx("span", { style: pageStyle.label, children: t("settings.rootDir") }), react_jsx_runtime.jsx("input", {
									style: pageStyle.input,
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const injectProps = () => {
					...
					// 状态页前缀: 跟随设置中的 statusPath (默认 /wechat/status), 避免硬编码
					const statusPath = String(value?.statusPath || "/wechat/status").replace(/\/+$/, "");
					...
					return { t: (key) => (zh[key] ?? en[key] ?? key), value, save, saving, statusPath };
				};
