...
async function applyModelOverrideToSession(ctx, sessionId, override) {
  ...
  const payload = {
    ...
    ...(override.reasoningEffort ? { reasoningEffort: String(override.reasoningEffort) } : {}),
  }
  let applied = false
...
async function tryHandleSwitch(ctx, chatId, text) {
  ...
  if (pending && Date.now() < pending.expiresAt) {
    ...
    if (pending.kind === "effort") {
      ...
      await updateBinding(ctx, chatId, { provider: m.provider, model: m.id, reasoningEffort: eff.id })
      diag.lastSwitch = { at: Date.now(), chatId, action: "switch-model", provider: m.provider, model: m.id, reasoningEffort: eff.id }
      // 目标会话已 live 时立即推送会话级模型选择 (GUI 即时同步, 无需等下条消息)
      try {
        const b2 = resolveBinding(chatId)
        const sid2 = b2 === null ? null : resolveBindingSessionId(ctx, b2)
        if (sid2 !== null) {
          const pushed = await applyModelOverrideToSession(ctx, sid2, { provider: m.provider, model: m.id, reasoningEffort: eff.id })
          if (pushed) appliedOverrideKey.set(chatId, `${m.provider}/${m.id}/${eff.id}`)
        }
