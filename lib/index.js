...
function formatSessionList(wsList) {
  ...
  for (const ws of wsList) {
    ...
    if (sessions.length === 0) {
      lines.push(`${++n}. 📁 ${ws.path} (暂无会话)`)
      continue
    }
    for (const s of sessions) {
      lines.push(`${++n}. 📁 ${ws.path} → 会话「${s.title}」`)
    }
  }
...
function matchSessionChoice(wsList, text) {
  ...
  if (/^\d+$/.test(t)) {
    ...
    for (const ws of wsList) {
      for (const s of ws.sessions ?? []) {
        n += 1
        if (n === Number(t)) return { workspace: ws.path, sessionTitle: s.title }
      }
    }
    return null
  }
  ...
  for (const ws of wsList) {
    for (const s of ws.sessions ?? []) {
      if (norm(s.title) === tn) return { workspace: ws.path, sessionTitle: s.title }
    }
  }
  for (const ws of wsList) {
    for (const s of ws.sessions ?? []) {
      const sn = norm(s.title)
      if (sn.includes(tn) || tn.includes(sn)) return { workspace: ws.path, sessionTitle: s.title }
    }
  }
  return null
}
...
function findExactSession(wsList, workspace, sessionTitle) {
  ...
  for (const ws of wsList) {
    ...
    for (const s of ws.sessions ?? []) {
      if (String(s.title ?? "").trim() === tn) return { workspace: ws.path, sessionTitle: s.title }
    }
  }
  return null
}
...
async function classifySwitch(ctx, text, bindingDesc, listText) {
  ...
  try {
    const prompt = [
      ...
      "",
      "注意: workspace 和 sessionTitle 必须与列表中完全一致 (同名标题可能出现在多个工作区, 重复时必须选对 workspace)。",
    ].join("\n")
...
async function tryHandleSwitch(ctx, chatId, text) {
  ...
  if (pending && Date.now() < pending.expiresAt) {
    ...
    if (chosen !== null) {
      ...
      await sendMessage(chatId, `✅ 已切换: ${chosen.workspace} → 会话「${chosen.sessionTitle}」\n之后的微信消息都会进入这个会话。`).catch(() => {})
      return true
    }
    // 未匹配: 重列列表, 保留 pending
  ...
  if (targetWs.length > 0) {
    ...
    await sendMessage(chatId, `✅ 已切换到工作区: ${decision.workspace}\n该工作区暂无会话, 下一条微信消息将自动创建新会话。`).catch(() => {})
    return true
  }
  // 指定的工作区不存在 → 列出全部
