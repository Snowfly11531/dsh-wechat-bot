...
/** updatesBuf 持久化: 重启后不再让服务器重放未确认的旧消息 (否则旧消息反复注入会话)。 */
const UPDATES_BUF_PATH = join(process.env.DSH_HOME ?? join(homedir(), ".dsh"), "data", "dsh-wechat-bot", "updates-buf.json")
function loadUpdatesBuf() {
  try { return readFileSync(UPDATES_BUF_PATH, "utf8") } catch { return "" }
}
function saveUpdatesBuf() {
  try {
    mkdirSync(dirname(UPDATES_BUF_PATH), { recursive: true })
    const tmp = `${UPDATES_BUF_PATH}.tmp`
    writeFileSync(tmp, updatesBuf, "utf8")
    renameSync(tmp, UPDATES_BUF_PATH)
  } catch { /* 持久化失败不阻塞 */ }
}
updatesBuf = loadUpdatesBuf()
  ...
  const poll = async () => {
    ...
    try {
      ...
      for (const msg of msgs) {
        ...
        await handleWechatMessage(bridgeCtx, { chatId, from: fromName, msg })
      }
      // 仅当本批没有 msgs (item_list 权威通道) 时, 才回退处理 deprecated 的 updates 通道,
      // 避免同一条消息被两个通道各注入一次 (造成会话重复/串味)。
      if (msgs.length === 0) for (const update of updates) {
        ...
        await handleWechatMessage(bridgeCtx, { chatId, from: fromName, text })
      }
...
/**
 * 微信消息统一入口 (按用户要求的管线):
 *   ① 收消息 → ② 判语音/文字 (语音先转文字) → ③ 精确词判断 (切换指令)
 *   → ④ 未命中才注入会话 (桥接)。语音/文字走同一路径。
 * @param ctx - 插件上下文。
 * @param meta - { chatId, from, msg } (msg 为 item_list 结构, 含语音) 或 { chatId, from, text }。
 */
async function handleWechatMessage(ctx, meta) {
  const chatId = String(meta?.chatId ?? "")
  const fromName = String(meta?.from ?? "unknown")
  const msg = meta?.msg
  let text = ""

  // ① 判语音/文字: item_list 里有语音 → 先转文字
  const items = Array.isArray(msg?.item_list) ? msg.item_list : []
  if (items.some((item) => item?.type === WX_ITEM_VOICE)) {
    const voices = await transcribeVoiceItems(ctx, msg)
    try {
  ...
  if (text === "") {
    // 纯文字消息 (msgs 或 updates 通道)
    text = items
      .filter((item) => item.type === WX_ITEM_TEXT && item.text_item && item.text_item.text)
      .map((item) => item.text_item.text)
      .join("\n")
    if (text === "") text = String(meta?.text ?? "").trim()
  }
  if (!chatId || !text) return

  // ② 入 inbox (供 wechat_read_inbox 工具查询)
  ...
  // ③ 精确词判断 (语音转好的文字同样参与): 命中 → 由 tryHandleSwitch 处理并返回
  if (activeConfig.bridge) {
    let handled = false
    try {
      handled = await tryHandleSwitch(ctx, chatId, text)
    } catch (error) {
  ...
  // autoReply (仅 bridge 关闭时生效) 或 注入会话 (bridge 开启)
  if (!activeConfig.bridge) {
    if (autoReply) {
  ...
  // ④ 注入会话 (桥接): 同一 chat 的消息已由 bridgeQueues 串行化, 不同 chat 互不阻塞
  void bridgeMessage(ctx, { chatId, from: fromName, text }).catch((error) => {
 ...
 * 标题解析 live 会话事件优先, live 会话内无 session/title 事件时回退持久化日志 (带缓存) —
 * 修复: live 会话老标题事件不在内存 → 之前一直匹配不到 → 桥接/切换报"未匹配到会话"。
 * @returns sessionId 字符串; 找不到返回 null。
 */
async function resolveBindingSessionId(ctx, binding) {
  if (binding?.sessionId) return String(binding.sessionId)
  if (!binding?.workspace || !binding?.sessionTitle) return null
  const wantTitle = String(binding.sessionTitle ?? "").trim()
  if (!wantTitle) return null
  const norm = (p) => String(p ?? "").replace(/[\\/]+$/, "").replace(/\//g, "\\").toLowerCase()
  const wantPath = norm(binding.workspace)
  try {
    const registry = ctx.get("workspaceRegistry")
    const sessionsSvc = ctx.get("sessions")
    const persistence = ctx.get("sessionPersistence")
    const titleOf = async (sid) => {
      const live = sessionsSvc?.get(sid)
      let t = live ? sessionTitleOf(live) : ""
      if (t === "" && persistence !== void 0 && typeof persistence.loadStored === "function") {
        try { t = await storedTitleOf(persistence, sid) } catch { t = "" }
      }
      return t.trim()
    }
    // 1) 注册表: 按 工作区路径 + 标题 匹配
    if (registry !== void 0 && typeof registry.list === "function") {
      for (const ws of registry.list()) {
        if (norm(ws.path) !== wantPath) continue
        for (const sid of ws.sessionIds ?? []) {
          const s = String(sid)
          if ((await titleOf(s)) === wantTitle) return s
        }
      }
    }
    // 2) 兜底: 持久化会话按 cwd + 标题 (注册表挂载缺失/陈旧时也能找到)
    if (persistence !== void 0 && typeof persistence.list === "function") {
      const all = await persistence.list()
      for (const row of all ?? []) {
        const s = String(row?.id ?? "")
        if (!s) continue
        if (norm(row.cwd) !== wantPath) continue
        if ((await titleOf(s)) === wantTitle) return s
      }
    }
  } catch { /* fallthrough */ }
...
/** 会话是否存在于持久化存储 (sessions.get 只回 live, 持久化但未加载的会话要用这个判断)。 */
async function sessionExistsPersisted(ctx, sid) {
  const persistence = ctx.get("sessionPersistence")
  if (persistence === void 0 || typeof persistence.loadStored !== "function") return false
  try {
    const stored = await persistence.loadStored(sid)
    return stored !== void 0 && stored !== null
  } catch {
    return false
  }
}
...
async function applyModelSelectionToChat(ctx, chatId, m, effId) {
  ...
  try {
    ...
    const sid2 = b2 === null ? null : await resolveBindingSessionId(ctx, b2)
    if (sid2 !== null) {
...
async function bridgeMessage(ctx, { chatId, from, text }) {
  // ★ 精确词判断已在 handleWechatMessage 完成 (命中则不会走到这里); 此处直接注入会话。
  const agents = ctx.get("agents")
  const sessions = ctx.get("sessions")
  const defaultModel = ctx.get("agentDefaultModel")
  if (agents === void 0 || sessions === void 0 || defaultModel === void 0) {
  ...
  const run = prev.then(async () => {
    ...
    if (agent === void 0) {
      ...
      // 工作区+标题绑定, 但会话尚未创建 → 首次消息时在绑定工作区创建 (模式二「新建会话」)
      const useCreatePath = async () => {
        ...
        if (await sessionExistsPersisted(ctx, sessionId)) {
      ...
      if (binding?.sessionId) {
        // 固定 sessionId 绑定: 复用其上下文 (显式 id, 不存在则报错重绑)
        const targetSessionId = await resolveBindingSessionId(ctx, binding)
        ...
        if (liveAgent !== void 0) {
          agent = liveAgent
        } else if (await sessionExistsPersisted(ctx, sid)) {
          const { agent: resumed, dispose } = await agents.resume({
            resumeSessionId: sid,
            agentOptions,
            setup: setupBase,
        ...
        // 工作区+标题绑定: 已存在同名会话 → 复用; 尚未创建 → 首次消息时创建
        const targetSessionId = await resolveBindingSessionId(ctx, binding)
        if (targetSessionId !== null) {
          const sid = SessionId(targetSessionId)
          const liveAgent = agents.get(sid)
