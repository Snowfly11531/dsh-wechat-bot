...
const SWITCH_KEYWORDS = ["切换", "切到", "换到", "换个", "换工作区", "换会话", "切工作区", "切会话", "workspace", "session", "switch"]
function looksLikeSwitchRequest(text) {
  const lower = String(text ?? "").toLowerCase()
  return SWITCH_KEYWORDS.some((k) => lower.includes(k))
}

/** 待确认切换: 询问会话列表后等待用户回复序号/标题 (30 分钟有效)。 */
const pendingSwitch = new Map() // chatId -> { list, expiresAt }
...
function matchSessionChoice(wsList, text) {
  ...
  if (/^\d+$/.test(t)) {
    ...
    for (const ws of wsList) {
      for (const s of ws.sessions ?? []) {
        n += 1
        if (n === Number(t)) return { workspace: ws.path, workspaceTitle: ws.title, sessionTitle: s.title }
      }
    }
    return null
  }
  ...
  for (const ws of wsList) {
    for (const s of ws.sessions ?? []) {
      if (norm(s.title) === tn) return { workspace: ws.path, workspaceTitle: ws.title, sessionTitle: s.title }
    }
  }
  for (const ws of wsList) {
    for (const s of ws.sessions ?? []) {
      const sn = norm(s.title)
      if (sn.includes(tn) || tn.includes(sn)) return { workspace: ws.path, workspaceTitle: ws.title, sessionTitle: s.title }
    }
  }
  return null
}

/** 精确查找 工作区+标题 对应的会话 (标题可能重复, 必须 workspace 也匹配)。 */
function findExactSession(wsList, workspace, sessionTitle) {
  const wn = normPath(workspace)
  const tn = String(sessionTitle ?? "").trim()
  for (const ws of wsList) {
    if (normPath(ws.path) !== wn) continue
    for (const s of ws.sessions ?? []) {
      if (String(s.title ?? "").trim() === tn) return { workspace: ws.path, workspaceTitle: ws.title, sessionTitle: s.title }
    }
  }
  return null
}
...
/** 当前 chat 绑定描述 (给路由模型的上下文)。 */
function bindingDescription(chatId) {
  const b = (activeConfig.bindings ?? []).find((x) => x.chatId === chatId)
  if (!b) return "当前: 未绑定"
  if (b.sessionId) return `当前: 固定会话 ${b.sessionId}`
  if (b.workspace && b.sessionTitle) return `当前: 工作区 ${b.workspace} / 会话「${b.sessionTitle}」`
  if (b.workspace) return `当前: 工作区 ${b.workspace} (默认会话)`
  return "当前: 未绑定"
}

/** 提取回复中的 JSON 对象 (容忍 markdown 代码块与前后杂文)。 */
...
/**
 * 用默认模型做一次"是否切换请求"的路由判断。
 * 临时 agent, 用完即弃 (不持久化、不挂工作区, 不污染会话列表)。
 * @returns { action: "switch"|"ask", workspace, sessionTitle } | null
 */
async function classifySwitch(ctx, text, bindingDesc, listText) {
  const agents = ctx.get("agents")
  const defaultModel = ctx.get("agentDefaultModel")
  if (agents === void 0 || defaultModel === void 0) throw new Error("agents/agentDefaultModel 服务不可用")
  const selection = defaultModel.currentSelection()
  if (selection === void 0 || selection.provider === void 0 || selection.model === void 0) {
    throw new Error("默认模型选择不可用")
  }
  const agentOptions = { provider: selection.provider, model: selection.model }
  const setup = (agentCtx) => {
    installModelSelection(agentCtx, { current: selection, assembled: void 0 })
  }
  const sid = SessionId(`wechat-router-${randomUUID()}`)
  ...
  try {
    const prompt = [
      "你是 DSH (DeepSeek Harness) 的会话路由器。",
      "你的唯一任务: 判断用户消息是否在请求「切换工作区/会话」, 并输出结构化结果。",
      "",
      bindingDesc,
      "",
      "可用工作区与会话 (格式: 序号. 📁 名字 (路径) → 会话「标题」):",
      listText,
      "",
      "用户消息:",
      text,
      ...
      '- 不是切换请求: {"action":"none"}',
      '- 是切换请求且目标明确 (能从列表确定工作区和会话标题): {"action":"switch","workspace":"<列表中的完整路径>","sessionTitle":"<列表中的标题>"}',
      '- 是切换请求但只指明工作区或目标不明确: {"action":"ask","workspace":"<列表中的完整路径, 未指明则为空字符串>"}',
      "",
      "注意: workspace 必须填列表项括号内的完整路径, sessionTitle 与列表中完全一致 (同名标题可能出现在多个工作区, 重复时必须选对 workspace)。",
    ].join("\n")
    ...
    if (parsed && typeof parsed === "object" && (parsed.action === "switch" || parsed.action === "ask")) {
      return {
        action: parsed.action,
        workspace: String(parsed.workspace ?? "").trim(),
        sessionTitle: String(parsed.sessionTitle ?? "").trim(),
      }
    }
    return null
  } finally {
...
/**
 * 处理"切换工作区/会话"请求 (拦截)。返回 true 表示消息已被处理, 不再进入正常桥接。
 * 流程: 待确认选择 → 关键词粗筛 → 默认模型判断 → 执行切换 / 列出会话询问。
 */
async function tryHandleSwitch(ctx, chatId, text) {
  // 1) 待确认状态: 上一轮被询问"切换到哪个会话", 用户回复序号/标题
  ...
  if (pending && Date.now() < pending.expiresAt) {
    ...
    if (chosen !== null) {
      ...
      diag.lastSwitch = { at: Date.now(), chatId, action: "switch", workspace: chosen.workspace, sessionTitle: chosen.sessionTitle }
      await sendMessage(chatId, `✅ 已切换: ${wsLabel(chosen)} → 会话「${chosen.sessionTitle}」\n之后的微信消息都会进入这个会话。`).catch(() => {})
      return true
    }
    // 未匹配: 重列列表, 保留 pending
  ...
  // 2) 关键词粗筛 (减少模型调用)
  if (!looksLikeSwitchRequest(text)) return false
  ...
  try {
    ...
    listText = formatSessionList(wsList)
  } catch {
    wsList = []
    listText = "(无法读取工作区列表)"
  }
  let decision = null
  try {
    decision = await classifySwitch(ctx, text, bindingDescription(chatId), listText)
  } catch (error) {
    // 模型判断失败 → 降级为正常桥接, 不让切换功能阻塞聊天
    lastError = `classify: ${error instanceof Error ? error.message : String(error)}`
    return false
  }
  if (decision === null) return false

  diag.lastSwitch = { at: Date.now(), chatId, action: decision.action, workspace: decision.workspace, sessionTitle: decision.sessionTitle }

  if (decision.action === "switch") {
    const chosen = findExactSession(wsList, decision.workspace, decision.sessionTitle)
    if (chosen !== null) {
      applyBinding(ctx, chatId, { chatId, workspace: chosen.workspace, sessionTitle: chosen.sessionTitle })
      await sendMessage(chatId, `✅ 已切换: ${wsLabel(chosen)} → 会话「${chosen.sessionTitle}」\n之后的微信消息都会进入这个会话。`).catch(() => {})
      return true
    ...
    // 目标不存在: 列出可选并进入待确认
    await sendMessage(chatId, `未找到会话「${decision.sessionTitle}」, 当前可选:\n${listText}\n\n回复序号或标题即可切换。`).catch(() => {})
    pendingSwitch.set(chatId, { list: wsList, expiresAt: Date.now() + PENDING_SWITCH_TTL })
    return true
  }

  // action === "ask": 只指明工作区 (或未指明) → 询问具体会话并给列表
  const targetWs = decision.workspace
    ? wsList.filter((w) => normPath(w.path) === normPath(decision.workspace))
    : wsList
  if (targetWs.length > 0) {
    const list = sliceSessionList(targetWs)
    if (list.some((w) => (w.sessions ?? []).length > 0)) {
      await sendMessage(chatId, `📂 请选择要切换到的会话 (回复序号或标题):\n${formatSessionList(list)}`).catch(() => {})
      pendingSwitch.set(chatId, { list, expiresAt: Date.now() + PENDING_SWITCH_TTL })
      return true
    }
    // 该工作区没有任何会话 → 绑定工作区 (新建模式, 下条消息创建会话)
    applyBinding(ctx, chatId, { chatId, workspace: decision.workspace })
    await sendMessage(chatId, `✅ 已切换到工作区: ${wsLabel(targetWs[0])}\n该工作区暂无会话, 下一条微信消息将自动创建新会话。`).catch(() => {})
    return true
  }
  // 指定的工作区不存在 → 列出全部
  await sendMessage(chatId, `未找到工作区「${decision.workspace}」, 当前可选:\n${listText}\n\n回复序号或标题即可切换。`).catch(() => {})
  pendingSwitch.set(chatId, { list: wsList, expiresAt: Date.now() + PENDING_SWITCH_TTL })
  return true
}
