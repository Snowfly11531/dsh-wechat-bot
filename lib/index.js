...
let activeConfig = { statusPath: DEFAULT_STATUS_PATH, autoReply: "", pollMs: 1000, bridge: true, emptyReply: "", rootDir: DEFAULT_ROOT_DIR, bindings: [] }
...
/** updatesBuf 持久化: 重启后不再让服务器重放未确认的旧消息 (否则旧消息反复注入/游标错位漏收)。 */
...
// ---- 诊断状态 (状态页 /json 输出) ----
let diag = { pollCount: 0, lastPollAt: 0, lastPollMsgs: 0, lastPollError: null, lastPollResp: null, tokenExpiredAt: null, lastSwitch: null, lastRaw: null, lastBridge: null }
...
const SETTINGS_PATCH_KEYS = new Set(["statusPath", "autoReply", "bridge", "emptyReply", "rootDir", "bindings"])
...
function startPollLoop(bridgeCtx) {
  ...
  const poll = async () => {
    ...
    try {
      ...
      // 逐条、按序处理 (每条: 记录原始接收 → 语音/文字 → 入 inbox → 精确词 → 注入会话)
      for (const msg of msgs) {
        ...
        await processIncomingMessage(bridgeCtx, { chatId, from: fromName, msg })
      }
      // updates 通道为 deprecated 兼容格式: 仅当本批没有 msgs (权威通道) 时回退处理, 避免双注入
      if (msgs.length === 0) for (const update of updates) {
        ...
        await processIncomingMessage(bridgeCtx, { chatId, from: fromName, text })
      }
...
/**
 * 处理一条收到的微信消息 (文字): 按固定确定性顺序执行 —
 *   ① 记录原始接收 (diag.lastRaw: 何时/哪个chat/几个item/提取文本)  → 可回答"发没发"
 *   ② 提取文本 (updates 通道直接给 text, msgs 通道从 item_list 提取)
 *   ③ 空文本/无 chat → 丢弃并记原因
 *   ④ 入 inbox + 标记 received
 *   ⑤ bridge 关闭 → 仅 autoReply; bridge 开启 → 交给 bridgeMessage
 *      (其内部: 精确词判断 → 命中即处理并返回; 未命中 → 注入会话)
 * bridge 注入为 fire-and-forget, 不阻塞收消息轮询 (同一 chat 由 bridgeQueues 串行化)。
 */
async function processIncomingMessage(bridgeCtx, { chatId, from, msg, text: directText }) {
  const items = Array.isArray(msg?.item_list) ? msg.item_list : []
  let text = directText != null ? String(directText) : ""
  if (text === "") {
    text = items
  ...
  diag.lastRaw = {
    ...
    itemTypes: items.map((i) => i?.type).filter((t) => t != null),
    text: text.slice(0, 80),
  }
  // ③ 空消息丢弃
  if (!chatId || !text) {
    if (diag.lastRaw) diag.lastRaw.dropped = !chatId ? "no-chat" : "no-text"
    return
  }
  // ④ inbox + received
  inbox.push({ from, chatId, content: text, time: Date.now() })
  ...
  if (!activeConfig.bridge) {
    if (activeConfig.autoReply) {
      const reply = activeConfig.autoReply.replace("{content}", text).replace("{from}", from)
      await sendMessage(chatId, reply).catch(() => {})
    }
    return
  }
  void bridgeMessage(bridgeCtx, { chatId, from, text }).catch((error) => {
...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register({
    name: "wechat_bind",
    description: "把某个微信 chat_id 绑定到指定工作区 (workspace) 或现有会话 (sessionId)。绑定会持久化到 settings.yaml (重启后仍生效)。绑定后该微信聊天将复用目标会话/工作区, 而不是默认的独立会话。chatId 来自 wechat_read_inbox。返回 { ok, message }。",
    parameters: {
      type: "object",
      properties: {
    ...
    async execute(args) {
      ...
      // 与 /bind 端点共享同一路径 (applyBinding): 更新内存 + 持久化 settings.yaml + 重置桥接 agent
      applyBinding(ctx, chatId, entry)
      return result(true, `已绑定 ${chatId} → ${entry.sessionId ? `会话 ${entry.sessionId}` : `工作区 ${entry.workspace}`} (已持久化到 settings.yaml, 重启后仍生效)`)
    },
  })
