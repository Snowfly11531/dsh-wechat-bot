...
// ---- 运行时状态 ----
let botStatus = "idle" // idle | starting | waiting-scan | scanned | logged-in | error
...
let inbox = []
let activeConfig = { statusPath: DEFAULT_STATUS_PATH, autoReply: "", pollMs: 1000, bridge: true, workspace: "", defaultWorkspace: "", bindings: [] }
...
// ---- 诊断状态 (状态页 /json 输出) ----
let diag = { pollCount: 0, lastPollAt: 0, lastPollMsgs: 0, lastPollError: null, lastPollResp: null }
    ...
    try {
      ...
      if (resp.errcode !== undefined && resp.errcode !== 0) {
        ...
        if (resp.errcode === -14 || msg.includes("session timeout")) {
          ...
          botStatus = "idle"
          lastError = "微信会话超时 (token 失效), 请到状态页重新扫码登录"
      ...
      for (const msg of msgs) {
        ...
        if (activeConfig.bridge) {
          await bridgeMessage(bridgeCtx, { chatId, from: fromName, text }).catch((error) => {
...
async function bridgeMessage(ctx, { chatId, from, text }) {
  ...
  const run = prev.then(async () => {
    ...
    if (agent === void 0) {
      ...
      } else if (binding?.workspace && binding?.sessionTitle && resolveBindingSessionId(ctx, binding) !== null) {
        // 工作区+标题绑定, 且已存在同名会话 → 复用 (动态解析)
        const sid = SessionId(resolveBindingSessionId(ctx, binding))
        const liveAgent = agents.get(sid)
        if (liveAgent !== void 0) {
          agent = liveAgent
        } else {
    ...
    const reply = collectReply(agent.session.events, fromSeq)
    if (reply !== "") {
  ...
  bridgeQueues.set(chatId, run.then(() => undefined, () => undefined))
  return run
}
...
<style>
  ...
  .badge.idle, .badge.error { background: #fee2e2; color: #b91c1c; }
...
const STATUS_TEXT = { idle: "未登录", starting: "启动中…", "waiting-scan": "等待扫码", scanned: "已扫码, 请在手机上确认", "logged-in": "已登录", error: "错误" };
...
function render(s) {
  ...
  if (s.status === "waiting-scan" && s.qrcode) {
    ...
    $("qrcode").innerHTML = "";
    $("hint").textContent = s.status === "logged-in" ? "已登录, 消息将自动接入 DSH" : "";
  }
        ...
        if (registry !== void 0 && typeof registry.list === "function") {
          for (const ws of registry.list()) {
            ...
            for (const h of allHeaders) {
              ...
              // 内存中无此会话 → 从持久化读取标题 (loadStored 返回含 events 的对象)
            ...
            for (const sid of ws.sessionIds ?? []) {
              ...
              if (title === "" && persistence !== void 0 && typeof persistence.loadStored === "function") {
                try {
                  title = sessionTitleOf(await persistence.loadStored(sidStr))
                } catch {
                  // ignore
                }
              }
