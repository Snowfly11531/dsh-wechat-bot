...
function persistBindings(entries) {
  ...
  const block = [
    ...
    ...entries.map((e) => [
      ...
      ...(e.sandboxMode ? [`      sandboxMode: '${esc(e.sandboxMode)}'`] : []),
    ].join("\n")),
  ].join("\n")
...
const SWITCH_WORDS = ["切换会话", "切换工作区", "切换模型", "切换权限"]
...
/** 权限模式选项 (与 DSH SandboxMode 一致)。 */
const SANDBOX_MODES = [
  { id: "read-only", name: "只读" },
  { id: "workspace-write", name: "工作区写入" },
  { id: "danger-full-access", name: "所有权限" },
]
  ...
  try {
    ...
    for (const p of providers) {
      ...
      for (const m of models) {
        out.push({ provider: pid, id: String(m.id ?? ""), name: String(m.name ?? m.id ?? "") })
      }
    }
  } catch {
    // ignore
...
/**
 * 更新当前 chat 的绑定字段 (保留 workspace/sessionId, 只改模型/权限等)。
...
async function tryHandleSwitch(ctx, chatId, text) {
  ...
  if (pending && Date.now() < pending.expiresAt) {
    ...
    if (pending.kind === "model") {
      ...
      pendingSwitch.set(chatId, { kind: "effort", model: m, expiresAt: Date.now() + PENDING_SWITCH_TTL })
      await sendMessage(chatId, `📂 已选模型 ${m.provider} / ${m.name}, 请选择推理等级 (回复序号或名称):\n${REASONING_EFFORTS.map((e, i) => `${i + 1}. ${e.name}`).join("\n")}`).catch(() => {})
      return true
    }
    if (pending.kind === "effort") {
      // 选推理等级 → 应用模型+等级
      const eff = matchModelChoice(REASONING_EFFORTS, text)
      if (eff === null) {
        await sendMessage(chatId, `没有找到「${String(text).trim()}」对应的推理等级, 请回复序号或名称:\n${REASONING_EFFORTS.map((e, i) => `${i + 1}. ${e.name}`).join("\n")}`).catch(() => {})
        return true
      }
      pendingSwitch.delete(chatId)
      const m = pending.model
      const entry = updateBinding(ctx, chatId, { provider: m.provider, model: m.id, reasoningEffort: eff.id })
      diag.lastSwitch = { at: Date.now(), chatId, action: "switch-model", provider: m.provider, model: m.id, reasoningEffort: eff.id }
      await sendMessage(chatId, `✅ 已切换模型: ${m.provider} / ${m.name}\n推理等级: ${eff.name}\n之后的微信消息将使用该模型。`).catch(() => {})
      return true
    }
    if (pending.kind === "sandbox") {
      // 选权限模式 → 应用
      const mode = matchModelChoice(SANDBOX_MODES, text)
      if (mode === null) {
        await sendMessage(chatId, `没有找到「${String(text).trim()}」对应的权限模式, 请回复序号或名称:\n${SANDBOX_MODES.map((m, i) => `${i + 1}. ${m.name} (${m.id})`).join("\n")}`).catch(() => {})
        return true
      }
      pendingSwitch.delete(chatId)
      const entry = updateBinding(ctx, chatId, { sandboxMode: mode.id })
      // 若该 chat 已有 live agent, 立即应用 (否则下条消息桥接时应用)
      try {
        const liveAgent = bridgeAgents.get(chatId) ?? ctx.get("agents")?.get(SessionId(resolveBindingSessionId(ctx, entry) ?? ""))
        if (liveAgent !== void 0) {
          setSandboxMode(liveAgent.session, mode.id)
          try { await ctx.get("sessions")?.flush?.(liveAgent.session) } catch { /* ignore */ }
        }
      } catch { /* ignore */ }
      diag.lastSwitch = { at: Date.now(), chatId, action: "switch-sandbox", sandboxMode: mode.id }
      await sendMessage(chatId, `✅ 已切换权限模式: ${mode.name} (${mode.id})\n之后的微信消息将在此权限下运行。`).catch(() => {})
      return true
    }
    // 兜底 (未知 kind): 清除并继续
    pendingSwitch.delete(chatId)
  ...
  if (switchWord === "切换权限") {
    await sendMessage(chatId, `📂 请选择权限模式 (回复序号或名称):\n${SANDBOX_MODES.map((m, i) => `${i + 1}. ${m.name} (${m.id})`).join("\n")}`).catch(() => {})
    pendingSwitch.set(chatId, { kind: "sandbox", expiresAt: Date.now() + PENDING_SWITCH_TTL })
    return true
  }
...
async function bridgeMessage(ctx, { chatId, from, text }) {
  ...
  const run = prev.then(async () => {
    ...
    if (agent === void 0) {
      ...
      // ★ 模型/推理等级: binding 里显式指定的优先, 否则用默认选择
    ...
    // ★ 应用绑定指定的权限模式 (会话级 sandbox 覆盖, 事件日志持久化)
    if (binding?.sandboxMode) {
      try {
        if (effectiveSandboxMode(agent.session.events ?? []) !== binding.sandboxMode) {
          setSandboxMode(agent.session, binding.sandboxMode)
        }
      } catch {
        // 权限应用失败不阻塞消息处理
      }
...
const SettingsConfig = z.object({
  ...
  // 除 chatId 外均为可选, 与 /bind 与 GUI 写入的数据结构一致; 模型/权限字段为可选覆盖
  bindings: z.array(z.object({
    ...
    sandboxMode: z.string().required(false),
  })).default([]),
})
