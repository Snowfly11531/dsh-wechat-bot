...
let loginPollGen = 0
let inbox = []
let activeConfig = { statusPath: DEFAULT_STATUS_PATH, autoReply: "", pollMs: 1000, bridge: true, workspace: "", defaultWorkspace: "", emptyReply: "", rootDir: "D:\\proj", bindings: [] }
...
async function createSession(ctx, workspacePath, name) {
  ...
  // ★ 挂载默认 agent preset (如梁神模式): 否则会话无任何工具 (bash/文件等)
  const presets = ctx.get("agentPresets")
  const presetId = (() => { try { return presets?.defaultId } catch { return void 0 } })()
  const setup = async (agentCtx) => {
    installModelSelection(agentCtx, { current: selection, assembled: void 0 })
    try {
      if (presets !== void 0 && typeof presets.mount === "function") await presets.mount(agentCtx)
    } catch { /* 挂载失败不阻塞 */ }
  }
...
async function tryHandleSwitch(ctx, chatId, text) {
  ...
  if (pending && Date.now() < pending.expiresAt) {
    ...
    if (pending.kind === "effort") {
      ...
      updateBinding(ctx, chatId, { provider: m.provider, model: m.id, reasoningEffort: eff.id })
...
async function bridgeMessage(ctx, { chatId, from, text }) {
  ...
  const run = prev.then(async () => {
    ...
    if (agent === void 0) {
      ...
      // ★ 默认 preset (梁神模式等): resume 已有会话不带, 新建会话才挂载
      const presets = ctx.get("agentPresets")
      const presetId = (() => { try { return presets?.defaultId } catch { return void 0 } })()
...
const SettingsConfig = z.object({
  ...
  // 工作区根目录: 「新增工作区」在此目录下创建同名文件夹并注册为 DSH 工作区
  rootDir: z.string().default("D:\\proj"),
...
export function apply(ctx, config = {}) {
  ...
  activeConfig = {
    ...
    rootDir: (config.rootDir ?? "D:\\proj").trim() || "D:\\proj",
